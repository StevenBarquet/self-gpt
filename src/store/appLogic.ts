...
import type { ICreateGptValues } from "src/pages/Home/HomeCont/CreateGpt/useCreateGptForm";
...
const actions: StateCreator<AppLogicStore> = (set) => ({
	...
	accumulateAiAnswer: (msg) =>
		set((state) => ({ ...state, aiAnswer: (state.aiAnswer += msg) })),
});

// ------------BOILERPLATE-----

export const useAppLogicStore = create<AppLogicStore>()(
	devtools(actions, { name: "AppLogic" }),
);
