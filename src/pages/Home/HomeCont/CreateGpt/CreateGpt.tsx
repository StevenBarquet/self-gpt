		...
		<Frow vAlign="middle" className={style["CreateGpt"]}>
			<LabelGridInput colProps={basicResponsive(25)} label="Title">
				<FBasicInput
					formik={formik}
					valueName="name"
					placeholder="Short name for your GPT"
				/>
			</LabelGridInput>
			...
			<Fcol>
				<div className="validate">
					<Button
						disabled={!hasIcon}
						onClick={() => setIcon(formik.values.icon)}
					>
						Validate Icon ✅
					</Button>
			...
			<Fcol {...basicResponsive(50)}>
				<div className="hint">
					<p>
						<span>Hint:</span> The relevant and more important think in your
						GPT.
					</p>
					<p>
						Context is a previous conversation that the chat will be based on
						every time you create a Chat based on this GPT.
					</p>
					<p>
						Your context should be in JSON format and must follow the following
						schema <span>-{">"}</span>
					</p>
				</div>
			</Fcol>
