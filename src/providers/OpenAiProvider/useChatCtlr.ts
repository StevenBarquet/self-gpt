...
export function useChatCtlr() {
	...
	function getChatType(): IChatTypes {
		if (mainScreen === "gptConversation" && !selectedConversation)
			return "EMPTY_NEW";
		if (mainScreen === "gptConversation" && !!messages?.length)
			return "JUST_CREATED";
